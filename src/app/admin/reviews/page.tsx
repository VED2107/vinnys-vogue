...
export default async function AdminReviewsPage({
  ...
  let query = supabase
    .from("reviews")
    .select(
      "id, product_id, user_id, rating, review_text, is_verified, status, created_at, products(title)",
      { count: "exact" }
    )
  ...
  // Batch-lookup profile emails for all user_ids in the results
  const rawRows = (reviews ?? []) as unknown as Omit<ReviewRow, "profileEmail">[];
  ...
  const rows: ReviewRow[] = rawRows.map((r) => ({
    ...r,
    profileEmail: emailMap[r.user_id] ?? null,
  }));
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-3 text-[12px] text-muted">
                          <span>{r.profileEmail ?? "Unknown"}</span>
                          <span>
