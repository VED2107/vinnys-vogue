-- 0019_order_items_snapshot_fields.sql
-- Add product_name and image_url snapshot columns to order_items
-- so order history survives product edits/deletions.

-- 1. Add new columns (nullable to not break existing rows)
ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS product_name text,
  ADD COLUMN IF NOT EXISTS image_url text;

-- 2. Backfill existing rows from the products table
...
-- 3. Replace checkout_cart to snapshot product_name and image_url
...
BEGIN
  ...
  -- Insert order items WITH snapshot fields (product_name + image_url)
  INSERT INTO order_items (order_id, product_id, quantity, price, product_name, image_url)
  SELECT v_order_id, ci.product_id, ci.quantity, pr.price_cents / 100.0, pr.title, pr.image_path
  FROM cart_items ci
  JOIN products pr ON pr.id = ci.product_id
  WHERE ci.cart_id = v_cart_id;
